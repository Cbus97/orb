(function(){
  ...
  console.log('[Orb Parent] script loaded, session in storage:',
    sessionStorage.getItem('orb_session')?'FOUND':'empty',
  ...
  iframe.addEventListener('load',function(){
    setTimeout(function(){
      var session=sessionStorage.getItem('orb_session');
      if(session){
  ...
  window.addEventListener('message',function(e){
    ...
    if(e.data.type==='orb-save-session'){
      try{
        sessionStorage.setItem('orb_session',e.data.session);
        console.log('[Orb Parent] session saved');
      }catch(_){}
    }
    if(e.data.type==='orb-clear-session'){
      try{
        sessionStorage.removeItem('orb_session');
        console.log('[Orb Parent] session cleared');
      }catch(_){}
    }
    if(e.data.type==='orb-get-session'){
      try{
        const session=sessionStorage.getItem('orb_session');
