(function(){
  ...
  var _cookieMatch=document.cookie.match(/(?:^|;\s*)orb_session=([^;]*)/);
  console.log('[Orb Parent] script loaded, session in cookie:',
    _cookieMatch?'FOUND':'empty',
  ...
  iframe.addEventListener('load',function(){
    setTimeout(function(){
      var _m=document.cookie.match(/(?:^|;\s*)orb_session=([^;]*)/);
    var session=_m?decodeURIComponent(_m[1]):null;
      if(session){
  ...
  window.addEventListener('message',function(e){
    ...
    if(e.data.type==='orb-save-session'){
      try{
        /* Use cookie — survives Simvoly's page transitions */
        document.cookie='orb_session='+encodeURIComponent(e.data.session)+';path=/;max-age=300';
        console.log('[Orb Parent] session saved to cookie');
      }catch(_){}
    }
    if(e.data.type==='orb-clear-session'){
      try{
        document.cookie='orb_session=;path=/;max-age=0';
        console.log('[Orb Parent] session cookie cleared');
      }catch(_){}
    }
    if(e.data.type==='orb-get-session'){
      try{
        const match=document.cookie.match(/(?:^|;\s*)orb_session=([^;]*)/);
