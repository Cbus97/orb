(function(){
  ...
  /* ════════════════════════════════════════
     ...
     ════════════════════════════════════════ */
  if(!isMobile){
    var dragging=false, mouseOffsetX=0, mouseOffsetY=0;
    ...
    window.addEventListener('message',function(e){
      ...
      if(e.data.type==='orb-dragstart'){
        ensurePos();
        dragging=true;
        /* e.data.x/y = mousedown position within iframe (0-340, 0-420)
           mouseOffsetX = how far mouse is from wrap left edge
           On mousemove: newWrapLeft = mouseClientX - mouseOffsetX */
        mouseOffsetX=e.data.x;
        mouseOffsetY=e.data.y;
    ...
    document.addEventListener('mousemove',function(e){
      if(!dragging) return;
      var newLeft=Math.max(0,Math.min(window.innerWidth-340, e.clientX-mouseOffsetX));
      var newTop=Math.max(0,Math.min(window.innerHeight-420, e.clientY-mouseOffsetY));
      /* Only move if mouse actually moved from start position */
      if(!hasMoved){
        var dl=Math.abs(newLeft-curLeft), dt=Math.abs(newTop-curTop);
        if(dl<3&&dt<3) return;
        hasMoved=true;
      }
