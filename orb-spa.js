(function(){
  ...
  /* ════════════════════════════════════════
     ...
     ════════════════════════════════════════ */
  if(!isMobile){
    var dragging=false, startMouseX=0, startMouseY=0, startWrapLeft=0, startWrapTop=0;
    ...
    window.addEventListener('message',function(e){
      ...
      if(e.data.type==='orb-dragstart'){
        ensurePos();
        dragging=true;
        startMouseX=curLeft+e.data.x;
        startMouseY=curTop+e.data.y;
        startWrapLeft=curLeft;
        startWrapTop=curTop;
        console.log('[Drag] curLeft=',curLeft,'e.data.x=',e.data.x,'startMouseX=',startMouseX,'innerWidth=',window.innerWidth);
    ...
    document.addEventListener('mousemove',function(e){
      if(!dragging) return;
      /* New wrap position = start position + how far mouse moved */
      var newLeft=startWrapLeft+(e.clientX-startMouseX);
      var newTop=startWrapTop+(e.clientY-startMouseY);
      curLeft=Math.max(0,Math.min(window.innerWidth-340,newLeft));
      curTop=Math.max(0,Math.min(window.innerHeight-420,newTop));
      wrap.style.left=curLeft+'px';
      wrap.style.top=curTop+'px';
    });
