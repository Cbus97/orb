  ...
  if(isMobile){
    ...
    window.addEventListener('message',function(e){
      ...
      if(e.data.type==='orb-mobile-dragstart'){
        ensurePos();
        mDragging=true;
        /* Offset = where finger is relative to wrap top-left */
        mOffsetX=e.data.touchX-curLeft;
        mOffsetY=e.data.touchY-curTop;
        wrap.style.opacity='0.85';
      }
      if(e.data.type==='orb-mobile-dragmove'&&mDragging){
        /* New wrap position = finger position minus fixed offset */
        curLeft=Math.max(0,Math.min(window.innerWidth-340, e.data.touchX-mOffsetX));
        curTop=Math.max(0,Math.min(window.innerHeight-420, e.data.touchY-mOffsetY));
        wrap.style.left=curLeft+'px';
        wrap.style.top=curTop+'px';
      }
