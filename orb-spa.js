(function(){
  ...
  /* ════════════════════════════════════════
     ...
     ════════════════════════════════════════ */
  if(!isMobile){
    var dragging=false, lastMouseX=0, lastMouseY=0;
    ...
    window.addEventListener('message',function(e){
      ...
      if(e.data.type==='orb-dragstart'){
        ensurePos();
        dragging=true;
        /* Seed lastMouse with current mouse position — will be set on first mousemove */
    ...
    document.addEventListener('mousemove',function(e){
      if(!dragging) return;
      if(lastMouseX===null){
        /* First move after dragstart — just record position, no jump */
        lastMouseX=e.clientX;
        lastMouseY=e.clientY;
        return;
      }
      /* Move wrap by delta from last position */
      var dx=e.clientX-lastMouseX;
      var dy=e.clientY-lastMouseY;
      lastMouseX=e.clientX;
      lastMouseY=e.clientY;
      curLeft=Math.max(0,Math.min(window.innerWidth-340,curLeft+dx));
      curTop=Math.max(0,Math.min(window.innerHeight-420,curTop+dy));
      wrap.style.left=curLeft+'px';
      wrap.style.top=curTop+'px';
    });
