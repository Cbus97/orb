(function(){
  ...
  function getWrapLeft(){ return parseInt(wrap.style.left)||0; }
  function getWrapTop(){ return parseInt(wrap.style.top)||0; }
  ...
  function startDrag(clientX, clientY){
    ...
    startLeft=getWrapLeft(); startTop=getWrapTop();
  ...
  /* Mouse drag (desktop) via postMessage from iframe */
  window.addEventListener('message',function(e){
    if(!e.data) return;
    if(e.data.type==='orb-dragstart'){
      startDrag(getWrapLeft()+e.data.x, getWrapTop()+e.data.y);
  ...
  /* Touch drag (mobile) — directly on wrapper */
