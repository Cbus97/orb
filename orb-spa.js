(function(){
  ...
  function doMove(clientX,clientY){
    ...
    if(Math.abs(newLeft-curLeft)>8||Math.abs(newTop-curTop)>8) dragMoved=true;
    if(!dragMoved) return;
  ...
  window.addEventListener('message',function(e){
    ...
    if(e.data.type==='orb-dragstart'){
      ...
      /* offsetX/Y = mouse position within wrap — keeps orb under finger */
      offsetX=e.data.x;
      offsetY=e.data.y;
      document.body.style.userSelect='none';
      document.body.style.cursor='grabbing';
    }
    if(e.data.type==='orb-dragend') doStop();
