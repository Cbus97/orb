(function(){
  ...
  if(!isMobile){
    ...
    window.addEventListener('message',function(e){
      ...
      if(e.data.type==='orb-dragstart'){
        ...
        /* e.data.x/y = mousedown clientX/Y within iframe
           page mouse position = curLeft + e.data.x */
    ...
    document.addEventListener('mousemove',function(e){
      if(!dragging) return;
      /* Smooth delta-based movement */
      curLeft=Math.max(0,Math.min(window.innerWidth-340, startLeft+(e.clientX-startMouseX)));
      curTop=Math.max(0,Math.min(window.innerHeight-420, startTop+(e.clientY-startMouseY)));
      wrap.style.left=curLeft+'px';
      wrap.style.top=curTop+'px';
    });
