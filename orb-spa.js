(function(){
  ...
  /* ════════════════════════════════════════
     ...
     ════════════════════════════════════════ */
  if(isMobile){
    var mDragging=false, mPendingDx=0, mPendingDy=0, mRafPending=false;

    function applyMobileMove(){
      mRafPending=false;
      if(!mDragging) return;
      curLeft=Math.max(0,Math.min(window.innerWidth-340, curLeft+mPendingDx));
      curTop=Math.max(0,Math.min(window.innerHeight-420, curTop+mPendingDy));
      mPendingDx=0; mPendingDy=0;
      wrap.style.left=curLeft+'px';
      wrap.style.top=curTop+'px';
    }
    ...
    window.addEventListener('message',function(e){
      ...
      if(e.data.type==='orb-mobile-dragstart'){
        ensurePos();
        mDragging=true; mPendingDx=0; mPendingDy=0;
        wrap.style.opacity='0.85';
      }
      if(e.data.type==='orb-mobile-dragmove'&&mDragging){
        /* Accumulate deltas, apply once per animation frame */
        mPendingDx+=e.data.dx;
        mPendingDy+=e.data.dy;
        if(!mRafPending){
          mRafPending=true;
          requestAnimationFrame(applyMobileMove);
        }
      }
